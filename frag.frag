...
//      direction = p3d_LightSource[i].position.xyz;
      d = direction;
    }
    vec3 LIGHT_COLOR = p3d_LightSource[i].color.xyz;
    float ATTENUATION = p3d_LightSource[i].constantAttenuation;

    vec3 contribution = round(clamp(dot(NORMAL, direction) + 0.5, 0.0, 1.0)) * ATTENUATION * LIGHT_COLOR;
    DOT += dot(NORMAL, direction) + 0.3;
//    contribution = clamp(contribution, 0., 1.);
    ...
    mid *= mid;
    float intensity = smoothstep(mid - roughness * 0.5, mid + roughness * 0.5, RdotV) * mid * 0.5 * PI;
    SPECULAR_LIGHT += 1.0 * intensity * ATTENUATION * LIGHT_COLOR;
  }

  vec4 color = vec4(texture(p3d_Texture2, texcoord).rgb * (SPECULAR_LIGHT + DIFFUSE_LIGHT + AMBIENT_LIGHT), 1.);
//  color = vec4(d, 1);
//  color = vec4(NORMAL, 1);
